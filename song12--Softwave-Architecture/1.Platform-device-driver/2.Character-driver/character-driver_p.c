...
static int character_driver_p_probe(struct platform_device *pdev)
{
	int ret;
	...
	if (ret < 0) {
		printk(TAG "character_driver_p_probe *chrdev_region fail: %d.\n", ret);
		return ret;
	}
	...
	if (!globalmem_devp) {
		ret = -ENOMEM;
		printk(TAG "character_driver_p_probe kzalloc fail: %d.\n", ret);
	...
	if (ret < 0) {
		printk(TAG "character_driver_p_probe cdev_add fail: %d.\n", ret);
		return ret;
	}

	printk(TAG "character_driver_p_probe sucess\n");
	return 0;
...
static int character_driver_p_remove(struct platform_device *pdev)
{
	...
	printk(TAG "character_driver_p_remove sucess\n");
...
static struct platform_driver character_driver_p = {
	...
	.probe = character_driver_p_probe,
	.remove = character_driver_p_remove,
};

static int __init character_driver_p_init(void)
{
	...
	printk(TAG "character_driver_p_init sucess\n");
	return 0;
}

static void __exit character_driver_p_exit(void)
{
	//platform_device_unregister(&character_device1_p);
	platform_driver_unregister(&character_driver_p);

	printk(TAG "character_driver_p_exit sucess\n");
}

module_init(character_driver_p_init);
module_exit(character_driver_p_exit);
