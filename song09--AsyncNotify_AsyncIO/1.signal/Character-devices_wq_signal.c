...
#define DEVICE_NAME "character_devices_wq_signal"
#define TAG "[character_devices_wq_signal] "
...
static int __init character_devices_wq_signal_init(void)
{
	int ret;
	// int i;
	...
	if (globalfifo_major)							//init devno
		//ret = register_chrdev_region(devno, GLOBALFIFO_NUM, "character_devices_wq_signal-globalfifo_name1_n");
		ret = register_chrdev_region(devno, 1, "character_devices_wq_signal-globalfifo_name1_n");
	else {
		//ret = alloc_chrdev_region(&devno, 0, GLOBALFIFO_NUM, "character_devices_wq_signal-globalfifo_name2_n");
		ret = alloc_chrdev_region(&devno, 0, 1, "character_devices_wq_signal-globalfifo_name2_n");
		globalfifo_major = MAJOR(devno);
	}
	
	if (ret < 0) {
		printk(TAG "character_devices_wq_signal_init *chrdev_region fail: %d.\n", ret);
		return ret;
	}
	...
	if (!globalfifo_devp) {
		ret = -ENOMEM;
		printk(TAG "character_devices_wq_signal_init kzalloc fail: %d.\n", ret);
	...
	printk(TAG "character_devices_wq_signal_init sucess\n");
	return 0;
...
static void __exit character_devices_wq_signal_exit(void)
{
	//int i;
	...
	printk(TAG "character_devices_wq_signal_exit sucess\n");
}

module_init(character_devices_wq_signal_init);
module_exit(character_devices_wq_signal_exit);
