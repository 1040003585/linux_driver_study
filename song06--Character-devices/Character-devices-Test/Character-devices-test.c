...
#define DEVICE_NAME "character_devices_test"
#define TAG "[character_devices_test] "
...
static int character_devices_test_init(void)
{
	int ret;
...
*/
	...
	if (ret < 0) {
		printk(TAG "character_devices_test_init *chrdev_region fail: %d.\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		printk(TAG "character_devices_test_init cdev_add fail: %d.\n", ret);
		return ret;
	}

	printk(TAG "character_devices_test_init sucess\n");
	return 0;
...
static void character_devices_test_exit(void)
{
//	kfree(globalmem_devp);
	...
	printk(TAG "character_devices_test_exit sucess\n");
}

module_init(character_devices_test_init);
module_exit(character_devices_test_exit);
